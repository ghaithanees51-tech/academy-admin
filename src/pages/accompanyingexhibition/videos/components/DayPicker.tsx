...
import type { Day } from '../../../../services/accompanyingExhibitionVideosApi';

const STORAGE_KEY = 'accompanyingexhibition.videos.selectedDayId';
...
export function setStoredDayId(dayId: string): void {
  try {
    if (dayId) localStorage.setItem(STORAGE_KEY, dayId);
...
interface DayPickerProps {
  ...
  loading?: boolean;
  disabled?: boolean;
  /** RTL: use for label/option alignment */
  className?: string;
}
...
export function DayPicker({
  ...
  const handleChange = (dayId: string) => {
    onSelect(dayId);
    setStoredDayId(dayId);
  };
      ...
      >
        <div className="relative">
          ...
          >
            <Listbox.Options
              className="
                absolute z-50 mt-2 max-h-60 w-full overflow-auto rounded-xl border border-gray-200
              ...
              >
                {({ selected }) => (
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {selected && (
                      <Check className="h-4 w-4 flex-shrink-0" style={{ color: PRIMARY }} aria-hidden />
                    )}
                  </div>
                )}
              </Listbox.Option>
