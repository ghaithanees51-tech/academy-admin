...
import { getCompetitionPhotos, deleteCompetitionPhoto, type CompetitionPhoto } from '../../../services/competitionPhotosApi';
import { showToast } from '../../../utils/toast';
import Pagination from '../../../components/Pagination';
import { PHOTOS_PAGE_SIZE } from '../../../config/api';

/**
 * Competition Photos page: 2-column layout.
...
export default function CompetitionPhotosPage() {
  ...
  const fetchPhotos = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getCompetitionPhotos(selectedDayId || undefined, currentPage, pageSize);
      const list = res.results ?? [];
      setPhotos(list);
      const count = res.count ?? list.length;
      setTotalCount(count);
    } catch (err) {
      showToast.error(err instanceof Error ? err.message : t('competitionPhotos.failedToLoad') || 'Failed to load photos');
      setPhotos([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
  ...
  const handleDelete = useCallback(async (id: number): Promise<void> => {
    ...
    try {
      await deleteCompetitionPhoto(id);
    } catch (err) {
      setPhotos(previous);
      setTotalCount((prev) => prev + 1);
      showToast.error(err instanceof Error ? err.message : t('competitionPhotos.deleteFailed') || 'Delete failed');
      throw err;
    }
  }, [photos, t]);
  ...
  return (
    <div className="flex flex-col min-h-0">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('competitionPhotos.title') || 'Competition Photos'}</h1>
          <p className="text-gray-600 text-sm mt-0.5">{t('competitionPhotos.subtitle') || 'Manage competition photos by day'}</p>
        </div>
      </div>
