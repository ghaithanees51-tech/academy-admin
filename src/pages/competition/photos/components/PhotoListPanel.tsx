...
import type { CompetitionPhoto } from '../../../../services/competitionPhotosApi';
      ...
      try {
        ...
        canvas.toBlob(
          (jpegBlob) => {
            ...
            a.href = url;
            a.download = `competition-photo-${photoId}.jpg`;
            a.click();
...
function PhotoCard({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.download = `competition-photo-${photo.id}.jpg`;
      a.target = '_blank';
    ...
    return (
      <div className={className}>
        <div className="flex flex-col items-center justify-center py-16 px-4 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50">
          ...
          <p className="text-lg font-medium text-gray-600">{t('competitionPhotos.noPhotosYet') || 'No photos yet'}</p>
          <p className="text-sm text-gray-500 mt-1">{t('competitionPhotos.addUsingPanel') || 'Add photos using the panel on the right'}</p>
        </div>
      </div>
    );
  }
